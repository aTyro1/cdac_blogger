...
def loadArticles(request):
    ...
    return HttpResponse(article.render({'writer_name':writer_name,'date':a[0].date,'title':a[0].title,'blog':a[0].blog,'comments':comments,'blog_id':a[0].id,'writer':writer}))
...
def submit_article(request):
    ...
    b=blogs(title=blog_title,writer_id=request.session['id'],blog=blog_content,writer_name=verified_writer.objects.filter(writer_id=request.session['id'])[0].writer_name)
    b.save()
    new_submissions_html=loader.get_template('new_submissions.html')
